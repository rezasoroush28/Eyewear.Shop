...
public partial class AppDbContext : IProductRepository
{
    ...
    public async Task<Product> GetByIdAsyncTracking(int id, CancellationToken cancellationToken)
    {
        return await Products.Where(p => p.Id == id && !p.IsDeleted).FirstOrDefaultAsync();
    }

    public async Task<Product> GetByIdAsyncNoTracking(int id, CancellationToken cancellationToken)
    {
